        ...
        if (response.statusCode == 200) {
          if (decoded["status"] == 200) {
            ...
            if (data["changePassword"] == true) {
              emit(UserLoginResetPasswrodRequiredSuccess(userName: event.username));
            } else {
              ...
              if (role == 'alphaAdmin' ||
                ...
                LoginCredentialsBox.loginCredentialsBox.saveLoginCredentials = LoginCredentialsModel(userId: event.username, password: event.password, savedAt: DateTime.now());
                emit(UserLoginSuccess());
              } else {
                emit(UserLoginFailure(decoded['message'] ?? "Login failed! User can't allow to login"));
              }
            }
...
class UserLoginResetPasswrodRequiredSuccess extends UserLoginState {
  UserLoginResetPasswrodRequiredSuccess({required this.userName});
  final String userName;
}
